...
act_config_all = [[4,4],[2,4],[2,4]]
result_holder = []
best_setup_holder = []

for idx, act_prec in enumerate([2]):  
    #for maxval in [1]:
    temp_best = 0
    temp_setup = None
    ...
    class Conv2d_q(_ConvNd):    
        ...
        def _conv_forward(self, input, weight, coeff):
            ...
            input = quantization_n_formal(input,act_prec, act_max)
            #if(self.layer_idx != 0):
    ...
    SAVEPATH = "/home/shurui/FWNN/fixedpooltraining/fw_weights/tinyconv_qd_zdim64_retrained_"+str(act_prec)+"bit.pth"
    best_prec1 = 0
