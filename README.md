...
When running scripts that use QuickDraw datasets (All MobileNet and Tinyconv scripts), two paths need to be modified in order to access the generated dataset. The first one is inside the *load_dataset* function, the path inside the *'with open()'* needs to be modified to the actual path of the class names. The second one is the *data_root* variable 

### Workflow and usage
#### Uncompressed network training and weight pool generation
The first step is to train uncompressed networks to generate the 'pre-trained' weights that will be further used to generate weight pools. After training the next step is to generate the weight pool for this network by applying K-means clustering on the trained weights. The uncompressed model training and weight pool generation are combined into a single script. You just need to run the corresponding python script under 'Accuracy\accuracy_codes\original_model_training' to generate uncompressed weights and weight pools. By default three weight pools with sizes 32, 64 and 128. 

**Make sure to change the *PATH* variable to the path you want the neural network weights to be stored and change the *output_path* variable to the path you want to store the generated weight pool. **
...
Once the uncompressed weights and weight pools are generated, the next step is to compress the network using the proposed weight pool method. This step involves replacing the original weights with weight vectors in the weight pool and retraining the network to refine the weight vector assignment (see Figure 2 in the paper). The scripts for this generating weight pool networks are under 'Accuracy\accuracy_codes\fw_training' and named as *network_dataset_wp_zdim_auto.py*. There is an outer loop in the scripts (*for pool_size in []:*) that loops over the weight pool size (32/64/128) to generate weight pool networks for different weight pool sizes.

**There are three path variables that need to be modified:**
- Change the *cluster_path* variable to the actual path that stores the generated cluster centers.
- Change the *PATH* variable to the trained uncompressed weight.
- Change the *SAVEPATH* variable to the path you want to store the weight pool compressed weights.

#### Activation precision and lookup table precision sweep
In table 5 and table 6 of the main paper, the accuracy results for different lookup table bitwidth and activation bitwidth are reported. Corresponding scripts are provided under 'Accuracy\accuracy_codes\fw_training' to generate these results. *network_dataset_prec_sweep.py* are the scripts for sweeping activation bitwidths and *network_dataset_prec_sweep_lut.py* are the scripts for sweeping lookup table bitwidths. 
##### Activation precision sweep
There is a outer loop *for act_prec in []:* or *for idx, act_prec in enumerate([]):* which loops over different activation bitwidths. You can modify the list content to evaluate the network accuracy with different activation bitwidths. Retraining codes are included in the scripts to retrain the network for the target activation bitwidth. In the paper for some higher bitwidths retraining is not performed due to good accuracy. You can comment out the retraining codes in such cases. 
**Similarly, there are three path variables that need to be modified:** 
- Change the *cluster_path* variable to the actual path that stores the generated cluster centers.
- Change the *PATH* variable to the uncompressed weight.
- Change the *SAVEPATH* variable to the path you want to store the retrained weights for specific activation precisions.

##### Lookup table precision sweep
There is a loop (*for psumbw in []:*) that loops over different lookup table bitwidths. You can modify the list content to sweep over desired lookup table bitwidths. The default activation bitwidth is set to 8 as reported in the paper.
**There are two path variables that need to be modified**
- Change the *cluster_path* variable to the actual path that stores the generated cluster centers.
- Change the *PATH* variable to the uncompressed weight.

Note in order to accurately simulate the effect of lookup table bitwidth, the original Pytorch convolution function is replaced with a custom bit-serial processed convolution function. Due to the custom convolution and bit-serial processing, the lookup table precision sweep scripts are quite slow compared to other scripts.

### Expected results
The expected results are listed in table 4, table 5 and table 6 in the paper. Table 4 shows the results for weight pool network accuracy for different weight pool sizes. Table 5 shows the accuracy for different lookup table bitwidths. Table 6 shows the accuracy for different activation table bitwidths. 
