...
DEFAULT_TESTDATA_SET = 'conv_large'
ALL_TESTDATA_SETS = {}
NETWORK_LAYERS = {}
...
class TestSettings(ABC):
    ...
    OUTDIR = './TestData_fullnetwork/'
    #OUTDIR = 'D:/research/fixedweightNN/CMSIS_5-develop/CMSIS_5-develop/CMSIS/NN/Tests/UnitTest/TestCases/TestData/'
    ...
    def __init__(self, dataset, testtype, in_ch, out_ch, x_in, y_in, w_x, w_y, stride_x, stride_y, pad, randmin,
        ...
        self.headers_dir = self.OUTDIR + networkname + '/' #'resnet14_data/'
    ...
    def write_c_config_header(self, write_common_parameters=True):
        ...
        with open(filepath, "a+") as f:
            if not os.path.getsize(filepath): #check whether is empty file, if empty then write the header, otherwise don't write the header
...
class ConvSettings(TestSettings):
    ...
    def __init__(self, dataset, testtype, in_ch=32, out_ch=32, x_in=32, y_in=32, w_x=3, w_y=3, stride_x=1, stride_y=1,
        ...
        super().__init__(dataset, testtype, in_ch, out_ch, x_in, y_in, w_x, w_y, stride_x, stride_y, pad,
...
class PoolingSettings(TestSettings):
    ...
    def __init__(self, dataset, testtype, randmin=-7, randmax=7, channels=8, x_in=4, y_in=4, w_x=4, w_y=4,
                 stride_x=1, stride_y=1, batches=1, pad=False, relu6=False):
        super().__init__(dataset, testtype, channels, channels, x_in, y_in, w_x, w_y, stride_x, stride_y, pad,
...
class FullyConnectedSettings(TestSettings):
    ...
    def __init__(self, dataset, testtype,   in_ch=1, out_ch=1, x_in=1, y_in=1, w_x=1, w_y=1, stride_x=1, stride_y=1,
        ...
        super().__init__(dataset, testtype,   in_ch, out_ch, x_in, y_in, w_x, w_y, stride_x, stride_y, pad, randmin,
...
class SoftmaxSettings(TestSettings):
    ...
    def __init__(self, dataset, testtype,   x_in=5, y_in=1, randmin=-7, randmax=7):
        super().__init__(dataset, testtype,   1, 1, x_in, y_in, 1, 1, 1, 1, False, randmin,
                         randmax)
...
class SVDFSettings(TestSettings):
    ...
    def __init__(self, dataset, testtype,   batches=2, number_inputs=2, rank=8, memory_size=10, randmin=-4,
                 randmax=4, input_size=3, number_units=4, generate_bias=True):
        super().__init__(dataset, testtype,   1, 1, 1, 1, 1, 1, 1, 1, False, randmin,
...
def load_all_testdatasets():
    ...
    type_of_test = 'conv'
    dataset = 'basic'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=1, out_ch=1, x_in=5,
                                              y_in=8, w_x=2, w_y=4, stride_x=1, stride_y=1, pad=False,
                                              randmin=1, randmax=4)
    dataset = 'stride2pad1'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=1, out_ch=1, x_in=7,
                                              y_in=7, w_x=3, w_y=3, stride_x=2, stride_y=2, pad=True,
                                              randmin=1, randmax=4)
    dataset = 'kernel1x1'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=4, out_ch=17, x_in=15,
                                              y_in=15, w_x=1, w_y=1, stride_x=1, stride_y=1, pad=False,
                                              randmin=1, randmax=4, outminrange=-126, outmaxrange=127)
    dataset = 'conv_3'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=3, out_ch=1, x_in=10, y_in=49, w_x=4,
                                              w_y=10, stride_x=1, stride_y=2, pad=True, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127)
    dataset = 'conv_1_x_n_1'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=3, out_ch=3, x_in=5, y_in=5, w_x=2,
                                              w_y=1, stride_x=2, stride_y=1, pad=False, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=2)
    dataset = 'conv_1_x_n_2'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=3, out_ch=1, x_in=11, y_in=11, w_x=11,
                                              w_y=1, stride_x=1, stride_y=1, pad=True, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127)
    dataset = 'conv_1_x_n_3'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=1, out_ch=3, x_in=11, y_in=11, w_x=1,
                                              w_y=11, stride_x=1, stride_y=1, pad=True, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127)
    dataset = 'conv_2'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=2, out_ch=4, x_in=6, y_in=3, w_x=3,
    ...
    dataset = 'conv_large'  # batches > 2
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=32, out_ch=32, x_in=32, y_in=32, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=False, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=1)

    dataset = 'conv_large_2'  # batches > 2
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=128, out_ch=128, x_in=32, y_in=32, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=False, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=1)
    
    dataset = 'conv_1616_3232'  # batches > 2
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=32, out_ch=32, x_in=16, y_in=16, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=False, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=1)

    dataset = 'conv_benchmark'  # batches > 2
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=32, out_ch=32, x_in=16, y_in=16, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=True, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=1)

    dataset = 'conv_small'  # batches > 2
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=8, out_ch=8, x_in=8, y_in=8, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=False, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=1)

    dataset = 'conv_out_activation'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=2, out_ch=2, x_in=3, y_in=3, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=True, randmin=-5, randmax=5,
                                              out_activation_min=-55, out_activation_max=55)

    dataset = 'conv_full_layer4'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=32, out_ch=32, x_in=4, y_in=4, w_x=3,
                                              w_y=3, stride_x=1, stride_y=1, pad=False, randmin=-5, randmax=5,
                                              out_activation_min=-127, out_activation_max=127)

    type_of_test = 'depthwise_conv'
    dataset = 'depthwise_2'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=3, out_ch=9, x_in=6, y_in=5, w_x=3,
                                              w_y=4, stride_x=2, stride_y=2, pad=True, randmin=-2, randmax=2,
                                              outminrange=-126, outmaxrange=127)
    dataset = 'depthwise_kernel_3x3'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=5, out_ch=5, x_in=4, y_in=5, w_x=3,
                                              w_y=3, stride_x=2, stride_y=2, pad=True, randmin=-2, randmax=2,
                                              outminrange=-126, outmaxrange=127)
    dataset = 'depthwise_eq_in_out_ch'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=6, out_ch=6, x_in=4, y_in=5, w_x=2,
                                              w_y=3, stride_x=1, stride_y=1, pad=True, randmin=-2, randmax=2,
                                              outminrange=-126, outmaxrange=127)
    dataset = 'depthwise_out_activation'
    ALL_TESTDATA_SETS[dataset] = ConvSettings(dataset, type_of_test,   in_ch=3, out_ch=3, x_in=6, y_in=5, w_x=3,
    ...
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=10, out_ch=6, x_in=2, y_in=1,
                                                        w_x=2, w_y=1, batches=3, input_zero_point=-50,
                                                        output_zero_point=-2)
    dataset = 'fully_connected_mve_0'
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=16, out_ch=9, x_in=1, y_in=1,
                                                        input_zero_point=-3, w_x=1, w_y=1, batches=1,
                                                        output_zero_point=-2)
    dataset = 'fully_connected_mve_1'
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=20, out_ch=4, x_in=1, y_in=1,
                                                        input_zero_point=-1, w_x=1, w_y=1, batches=1,
                                                        output_zero_point=3)
    dataset = 'fully_connected_null_bias_0'
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=33, out_ch=5,
                                                        input_zero_point=-1, batches=2, generate_bias=False)
    dataset = 'fully_connected_out_activation'
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=10, out_ch=4, randmin=-15,
                                                        randmax=15, input_zero_point=0, output_zero_point=0,
                                                        out_activation_min=-105, out_activation_max=120)

    dataset = 'fully_connected_debug'
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=128, out_ch=128, x_in=1, y_in=1,
                                                        w_x=1, w_y=1, batches=1, input_zero_point=-50,
                                                        output_zero_point=-2)

    dataset = 'fc_full_layer1'
    ALL_TESTDATA_SETS[dataset] = FullyConnectedSettings(dataset, type_of_test,   in_ch=128, out_ch=10, x_in=1, y_in=1,
                                                        w_x=1, w_y=1, batches=1, input_zero_point=-50,
                                                        output_zero_point=-2)                                                        
    type_of_test = 'avgpool'
    dataset = 'avgpooling'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=8, x_in=22, y_in=12, stride_x=9,
                                                 stride_y=5, w_x=6, w_y=5, pad=True)
    dataset = 'avgpooling_1'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=3, x_in=9, y_in=5, stride_x=1,
                                                 stride_y=2, w_x=9, w_y=5, pad=False)
    dataset = 'avgpooling_2'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=5, x_in=12, y_in=1, stride_x=1,
                                                 stride_y=2, w_x=3, w_y=1, pad=True)
    dataset = 'avgpooling_3'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=2, x_in=9, y_in=1, stride_x=2,
                                                 stride_y=1, w_x=1, w_y=1, pad=False)
    dataset = 'avgpooling_4'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=2, x_in=1, y_in=20, stride_x=1,
                                                 stride_y=3, w_x=1, w_y=3, pad=True)
    dataset = 'avgpooling_5'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   randmin=0, channels=1, x_in=3, y_in=3,
    ...
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=20, x_in=1, y_in=1, stride_x=1,
                                                 stride_y=1, w_x=1, w_y=1, pad=True)
    dataset = 'maxpooling_6'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=17, x_in=1, y_in=5, stride_x=1,
                                                 stride_y=3, w_x=3, w_y=4, pad=True)
    dataset = 'maxpooling_7'
    ALL_TESTDATA_SETS[dataset] = PoolingSettings(dataset, type_of_test,   channels=1, x_in=4, y_in=2, stride_x=2,
                                                 stride_y=2, w_x=2, w_y=2, pad=False, randmin=-20, randmax=-5,
                                                 relu6=True)
    type_of_test = 'softmax'
    dataset = 'softmax'
    ALL_TESTDATA_SETS[dataset] = SoftmaxSettings(dataset, type_of_test,   x_in=5, y_in=1)

    type_of_test = 'svdf'
    dataset = 'svdf'
    ALL_TESTDATA_SETS[dataset] = SVDFSettings(dataset, type_of_test,    batches=2, number_inputs=2, rank=8,
                                              memory_size=8, input_size=3, number_units=3)
    type_of_test = 'svdf'
    dataset = 'svdf_1'
    ALL_TESTDATA_SETS[dataset] = SVDFSettings(dataset, type_of_test,    batches=3, number_inputs=2, rank=1,
                                              memory_size=2, input_size=7, number_units=5)

    type_of_test = 'svdf'
    dataset = 'svdf_2'
    ALL_TESTDATA_SETS[dataset] = SVDFSettings(dataset, type_of_test,    batches=3, number_inputs=2, rank=2,
                                              memory_size=2, input_size=7, number_units=5, generate_bias=False)

    type_of_test = 'svdf'
    dataset = 'svdf_3'
    ALL_TESTDATA_SETS[dataset] = SVDFSettings(dataset, type_of_test,    batches=1, number_inputs=2, rank=1,
                                              memory_size=2, input_size=20, number_units=12, generate_bias=False)


def creat_layers(layer_list):
    #create the generator for all conv layers
    cnt = 1
    for layer in layer_list:
        dataset = 'convlayer' + str(cnt)
        NETWORK_LAYERS[dataset] = ConvSettings(dataset, 'conv',  in_ch=layer[2], out_ch=layer[3], x_in=layer[0], y_in=layer[0], w_x=layer[1],
                                              w_y=layer[1], stride_x=1, stride_y=1, pad=True, randmin=-2, randmax=2,
                                              outminrange=-127, outmaxrange=127, batches=1)
        cnt = cnt+1
...
#network info format is [input size, filter size (one dimension), number of channels, number of filters]


networkname = 'benchmarklayers'
selected_network = benchmarklayers
...
if __name__ == '__main__':
    ...
    #testdataset_list = ['conv_benchmark' , 'conv_small']
    test_type = 'conv' #check generate_test_data.py for all options of test_type arguments

    creat_layers(selected_network)
    cnt = 1
    for layer in selected_network:
        layername = 'convlayer' + str(cnt)
        try:
            generator = NETWORK_LAYERS[layername]
        except KeyError:
            print("WARNING: testset {} not in testset list".format(layername))
            if test_type == 'conv' or test_type == 'depthwise_conv':
                generator = ConvSettings(layername, test_type)
            elif test_type == 'fully_connected':
                generator = FullyConnectedSettings(layername, test_type)
            elif test_type == 'avgpool' or test_type == 'maxpool':
                generator = PoolingSettings(layername, test_type)
