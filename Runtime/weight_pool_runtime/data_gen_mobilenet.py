...
def creat_layers(layer_list, input_list):
    #create the generator for all conv layers
    cnt = 1
    for layer in layer_list:
        input_info = input_list[cnt-1]
        dataset = 'convlayer' + str(cnt)
        if layer[0] == 'c':
            #normal conv layers, including point-wise convolution
            NETWORK_LAYERS[dataset] = ConvSettings(dataset, 'conv',  in_ch=layer[2], out_ch=layer[1], x_in=input_info[0], y_in=input_info[0], w_x=layer[3],
                                                w_y=layer[3], stride_x=input_info[1], stride_y=input_info[1], pad=True, randmin=-2, randmax=2,
                                                outminrange=-127, outmaxrange=127, batches=1)
        else:
            #depthwise layers
            NETWORK_LAYERS[dataset] = ConvSettings(dataset, 'depthwise_conv',  in_ch=layer[2], out_ch=layer[1], x_in=input_info[0], y_in=input_info[0], w_x=layer[3],
                                                w_y=layer[3], stride_x=input_info[1], stride_y=input_info[1], pad=True, randmin=-2, randmax=2,
                                                outminrange=-127, outmaxrange=127, batches=1)         
        cnt = cnt+1


mobilenet_v2_weight = [ ['c', 32, 3, 3] ,
                        ['c', 32, 32, 1] ,
                        ['d', 32, 32, 3] ,
                        ['c', 16, 32, 1] ,
                        ['c', 16, 32, 1] ,
...
mobilenet_v2_input = [  [32, 1] ,
                        ...
                        [4, 1] ,
                        [4, 1] ]
#mobilenet weight info format is [conv type (depthwise or normal) ,number of filters, number of channels, filter size]
#mobilenet input info format is [input size, stride]

networkname = 'mobilenet_v2'
selected_network = mobilenet_v2_weight
...
if __name__ == '__main__':
    ...
    creat_layers(mobilenet_v2_weight,mobilenet_v2_input)
    cnt = 1
