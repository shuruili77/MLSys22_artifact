...
arm_status arm_convolve_s8_fusedpooling(const cmsis_nn_context *ctx,
                           const cmsis_nn_conv_params *conv_params,
                           const cmsis_nn_per_channel_quant_params *quant_params,
                           const cmsis_nn_dims *input_dims,
                           const q7_t *input_data,
                           ...
                           q7_t *output_data)
{
    ...
    const uint16_t kernel_x = filter_dims->w;
    const uint16_t kernel_y = filter_dims->h;
    const uint16_t output_x = (output_dims->w)/2;
    const uint16_t output_y = (output_dims->h)/2;
    const uint16_t output_ch = output_dims->c;
    ...
    /* 2x2 pooling embedded. check i_out_x and i_out_y whether is odd or even. 
    store the result of even index (since start with 0), and compare to the stored for odd index
    results only writes to activation memory atodd index after complete the comparision*/
        ...
        for (i_out_ch = 0; i_out_ch < output_ch; i_out_ch++)
        {
            for (i_out_y = 0; i_out_y < output_y; i_out_y++)
            {
                for (i_out_x = 0; i_out_x < output_x; i_out_x++)
                {
                    ...
                    conv_out = MIN(conv_out, out_activation_max);
                    if(conv_out<0){
                        conv_out = 0;// apply ReLU to the output
                    }
                    if (i_out_y % 2 == 0){
                        //even y dimension, no need to compare result, just store it
                        if (i_out_x % 2 == 0){
                            //even case, store the result to tmp result
                            conv_out_tmp = conv_out;
                        }
                        else{
                            //odd case
                            if(conv_out > conv_out_tmp){
                                output_data[i_out_ch + (i_out_y/2 * output_x + i_out_x/2) * output_ch] = (int8_t)conv_out;
                            }
                            else{
                                output_data[i_out_ch + (i_out_y/2 * output_x + i_out_x/2) * output_ch] = (int8_t)conv_out_tmp;
                            }
                        }
                    }
                    else{
                        if (i_out_x % 2 == 0){
                            ...
                            //odd case
                            if(conv_out < conv_out_tmp){
                                conv_out = conv_out_tmp;
                            }
                            //Then compare with the stroed result of previous row, update if larger than it
                            if (conv_out > output_data[i_out_ch + (i_out_y/2 * output_x + i_out_x/2) * output_ch] ){
                                output_data[i_out_ch + (i_out_y/2 * output_x + i_out_x/2) * output_ch] = conv_out;
                            }
